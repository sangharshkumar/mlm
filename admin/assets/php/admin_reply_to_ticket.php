...
if ($ticket_status == "pending") {
   mysqli_query($conn,"UPDATE $tickets_tbl SET status = 'open',  WHERE ticket_id = '$ticket_id' ");
}
mysqli_query($conn, "UPDATE $tickets_tbl SET  last_reply_date = '$current_date' WHERE ticket_id = '$ticket_id' ");
$query = mysqli_query($conn, "INSERT INTO $ticket_messages_tbl (`ticket_id`, `ticket_creator`, `ticket_message`, `ticket_date`, `ticket_files`) 
                    VALUES ('$ticket_id','$admin_id','$message','$current_date','$files') ");
...
if (!$query) {
    ...
    <div class="col-12 single-note-item all-category">
